...
# SMC NAND Flash driver - NAND Flash Read Write

This example application shows how to use the NAND Flash driver to perform read and write operations on NAND Flash device.

## Description

This example uses the NAND Flash driver to communicate with the SMC based NAND Flash to perform write and read operations in Bare-Metal environment.
...
Path of the application within the repository is **apps/driver/smc_flash/nand_flash/nand_flash_read_write/firmware** .
...
2. The LED is turned ON when the value read from NAND Flash matches with the written data
